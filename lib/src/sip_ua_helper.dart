...
class SIPUAHelper extends EventManager {
  ...
  Future<bool> call(String target,
      {bool voiceonly = false, MediaStream mediaStream = null, List<String> headers = const []}) async {
    if (_ua != null && _ua.isConnected()) {
      ...
      logger.error('Not connected, you will need to register.', null, StackTraceNJ());
    }
    return false;
  }
  ...
  void start(UaSettings uaSettings) async {
    if (_ua != null) {
      logger.warn('UA instance already exist!, stopping UA and creating a one...');
      _ua.stop();
    }
    ...
    _settings = Settings();
    WebSocketInterface socket = WebSocketInterface(uaSettings.webSocketUrl, uaSettings.webSocketSettings);
